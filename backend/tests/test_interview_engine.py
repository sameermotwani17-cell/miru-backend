...
def _mock_results():
    return (
        ...
        patch("store.interview_results.save_interview_results"),
    )
...
class TestTranscriptRebuild:
    ...
    def test_rebuild_transcript_order(self):
        """Each turn contributes question_prompt → user_answer → interviewer_response.

        The question_prompt message is what gives the LLM recall of every
        question already asked, which is how repeated questions are prevented.
        """
        from services.interview_engine import _rebuild_transcript

        turns = [
            {
                "question_prompt": "Could you introduce yourself?",
                "user_answer": "Hi, I am Sameer.",
                "interviewer_response": "Hello, welcome.",
            },
            {
                "question_prompt": "Tell me about your background.",
        ...
        assert transcript == [
            ...
            {"role": "user", "content": "Hi, I am Sameer."},
            {"role": "assistant", "content": "Hello, welcome."},
            {"role": "assistant", "content": "Tell me about your background."},
            {"role": "user", "content": "I worked at a startup."},
            {"role": "assistant", "content": "Thank you for that."},
        ]
...
class TestResultsEndpoint:
    ...
    def test_results_returns_required_fields(self):
        from routers.interview_results import _build_results_response
        ...
        with patch("routers.interview_results.get_session_turns", return_value=mock_turns), \
             patch("routers.interview_results.get_interview_results", return_value=mock_results):
    ...
    def test_missing_session_returns_error(self):
        ...
        with patch("routers.interview_results.get_interview_results", return_value=None):
...
class TestTimedCompletion:
    ...
    def test_time_expired_returns_interview_complete(self):
        ...
        with patch("services.interview_engine.get_session_turns", return_value=[]), \
             ...
             patch("services.feedback_engine.generate_full_feedback_package", return_value={}), \
             p_res_get, p_res_save:
            result = run_interview_turn(
                company="rakuten",
                language_mode="en",
    ...
    def test_debrief_triggered_on_time_expiry(self):
        ...
        mock_feedback = patch(
            "services.feedback_engine.generate_full_feedback_package",
            return_value={},
        )
