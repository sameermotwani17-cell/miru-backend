...
def _build_results_response(session_id: str) -> Dict[str, Any]:
    ...
    # Propagate the stored lifecycle status. The debrief engine writes
    # status="ready" into the cached results, but this builder used to drop it,
    # so the polling loop documented in the README ("poll until status=ready")
    # could never terminate — the frontend only worked because it fell back to
    # accepting a payload with no status at all.
    response_payload: Dict[str, Any] = {
        "session_id": session_id,
        "status": str(cached_results.get("status") or "ready"),
        "scores": radar_scores,
...
def get_feedback(session_id: str) -> Dict[str, Any]:
    # `turns` is the key the frontend client is typed against; `turn_feedback`
    # is the original key. Both are returned so neither side breaks.
    cached_results = get_interview_results(session_id)
    turn_feedback: List[Dict[str, Any]] = []
    if cached_results:
        candidate = cached_results.get("turn_feedback", [])
        if isinstance(candidate, list):
            turn_feedback = candidate
    return {"turns": turn_feedback, "turn_feedback": turn_feedback}
...
def get_transcript(session_id: str) -> Dict[str, Any]:
    ...
    if not turns:
        return JSONResponse(
            status_code=404, content={"detail": "Session not found", "turns": []}
        )
    transcript = _build_transcript(turns, [])
    return {"turns": transcript, "transcript": transcript}
